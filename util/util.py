...
def sliceData(originalAccDataDir, originalAudioDataDir, originalStartTime, interval, dest, start, frameLimit=40):
    ...
    # acc init
    originalAccData = pd.read_csv(originalAccDataDir)
...
def extractFeaturesToCSV(write, key, label):
    ...
    if not os.path.isfile(write):
        df = pd.DataFrame(None,
                          columns=['label', 'psd_x', 'psd_y', 'psd_z', 'fft_x', 'fft_y', 'fft_z', 'acf_x', 'acf_y',
                                   'acf_z', 'audio_mfcc', 'audio_fft'])
    ...
    for i in range(file_count):
        try:
            ...
            audio, sr = librosa.load('data/splitData/audio/audio' + key + '/' + 'audio' + key + '_' + str(i) + '.wav',
                                     sr=None)
...
def get_average_samples(audio_dir):
